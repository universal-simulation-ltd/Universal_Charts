# Launch a local preview of Universal Charts.
# Runs the dev server in the foreground — press Ctrl-C to stop.
# Windows equivalent of preview.sh.
#
#   Usage:  .\scripts\preview.ps1 [port]     (default 5185)
#
# 5185 is this app's port in the registry (Docs_UNI_SIM/dev-preview.md).
...
try {
    $port = if ($args.Count -ge 1) { $args[0] } else { '5185' }
    ...
    Write-Host "Universal Charts -> http://localhost:$port" -ForegroundColor Green
